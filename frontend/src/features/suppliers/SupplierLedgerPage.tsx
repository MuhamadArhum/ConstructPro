...
export default function SupplierLedgerPage() {
  ...
  const [addTransaction, { isLoading: adding }] = useAddSupplierTransactionMutation();
  const [deleteTransaction, { isLoading: deleting }] = useDeleteSupplierTransactionMutation();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTransaction({ supplierId: id, txId: deleteId }).unwrap();
  ...
  return (
    <Box>
      <AppBreadcrumbs crumbs={[{ label: 'Suppliers', to: '/suppliers' }, { label: data?.supplier?.name ?? '…' }, { label: 'Ledger' }]} />
      {/* Page Header */}
      <Stack direction="row" spacing={2} sx={{ alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ flex: 1 }}>
          <Typography variant="h1">SUPPLIER LEDGER</Typography>
          {supplier && (
            <Typography variant="body2" color="text.secondary">
              {supplier.name}{supplier.companyName ? ` — ${supplier.companyName}` : ''}{supplier.phone ? ` | ${supplier.phone}` : ''}
            </Typography>
          )}
        </Box>
        <Button
        ...
        <Grid size={{ xs: 12, sm: 4 }}>
          <Card variant="outlined">
            <CardContent>
              ...
              <Typography
                variant="h5"
                sx={{ fontWeight: 700, color: (supplier?.outstandingBalance ?? 0) > 0 ? 'error.main' : 'success.main' }}
              >
                {fmt(supplier?.outstandingBalance ?? 0)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
            ...
            <TableBody>
              {transactions.map((row) => (
                <TableRow key={row.id} hover>
                  ...
                  <TableCell>
                    <Chip
                      label={row.type}
                      color={row.type === 'PURCHASE' ? 'warning' : 'success'}
                      size="small"
                    />
                  </TableCell>
              ...
              {transactions.length > 0 && summary && (
                <TableRow sx={{ backgroundColor: 'action.hover' }}>
                  ...
                  <TableCell
                    align="right"
                    sx={{ fontWeight: 700, color: summary.closingBalance > 0 ? 'error.main' : 'success.main' }}
                  >
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            <Stack direction="row" spacing={2}>
              <FormControl fullWidth required>
                ...
                <Select label="Type" value={txType} onChange={(e) => setTxType(e.target.value as SupplierTransactionType)}>
