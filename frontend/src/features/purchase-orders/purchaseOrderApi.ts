...
export const purchaseOrderApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getPurchaseOrders: builder.query<{ data: PurchaseOrder[]; total: number }, { page?: number; pageSize?: number; status?: string; supplierId?: string; projectId?: string; search?: string }>({
